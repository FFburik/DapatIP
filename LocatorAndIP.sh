...
get_url=$(curl -s http://zlucifer.com/api/FFBurik.php?request=trap) #cek status
mulai(){
      ...
      if [ $lagi = "y" ];then
          ...
          echo "makasih Tod sudah menggunakan Trap Project"
          exit
      fi
}
start_trap_project(){
      echo "[1] Buat File Tod"
      echo "[2] Cek Hasil Tol"
      echo "[3] Close Trap Project Njing"
      echo "1/2/3?"
      read request
      if [ $request = "1" ]; then
            echo "Masukan nama file yang ingin Loh buat"
            read create
            ...
            echo "Masukan nama file yang sudah Loh buat"
